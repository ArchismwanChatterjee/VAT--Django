...
def ins(request):
    ...
    if user.objects.filter(email=u.email).exists():
        return render(request,'insert.html', {'message': 'Email already exists'})
    else:
        u.save()
        return render(request,'insert.html', {'message': 'User created successfully'})
...
def show(request):
    u=user.objects.all() # select * from user
...
def edit(request,id):
    u=user.objects.get(id=id)
    return render(request,'edit.html',{'x':u})
def del2(request,id):
...
def edit2(request, id):
    s = Student.objects.get(id=id)
    return render(request,'edit2.html',{'x':s})
def upload(request):
...
def upld(request):
    if request.method=='POST':
        ...
        with open('store/static/upload/'+str(request.FILES['a1']),'wb+') as destination:
            for c in request.FILES['a1'].chunks():
                destination.write(c)    
        url="upload/"+str(request.FILES['a1'])
        u=picfile()
        u.pname=str(request.FILES['a1'])
...
def log(request):
    ...
    if user.objects.filter(email=a,pwd=b):
        x= user.objects.get(email=a)
        return render(request,'Welcome.html',{'a': x})
    else:
